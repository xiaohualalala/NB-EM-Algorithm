...
List NB_MLE(NumericVector X, int iterMax = 15000, double EPSILON = 1e-7, bool verbose = false, bool se = false, bool rate = false)
{
    ...
    {
        NumericVector prevParam = NumericVector::create(_["esAvg"] = esAvg, _["eSize"] = eSize);
        ...
        if (std::isnan(esAvg) || std::isnan(eSize))
        {
        ...
        if (eProb >= 1 || eProb <= 0 || eSize <= 0)
        {
            ...
            eProb = mean(X) / var(X);
            eSize = mean(X) * mean(X) / (var(X) - mean(X));
        }
        ...
        // Convergence check
        NumericVector newParam = NumericVector::create(_["esAvg"] = esAvg, _["eSize"] = eSize);
