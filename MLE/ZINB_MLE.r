...
#'
ZINB_MLE <- function(X, iterMax = 15000, EPSILON = 1e-7, verbose = TRUE) {
    ...
    nonZeroX <- X[X > 0] # Subset of non-zero observations
    esAvg <- mean(nonZeroX) # Mean of non-zero observations
    esVar <- var(nonZeroX) # Variance of non-zero observations
