...
#' @param init_values Optional; a list of initial values for the parameters. Should contain `eProb` and `eSize`.
#'
...
#'
#' @export
NB_MLE <- function(X, iterMax = 1500, EPSILON = 1e-5, verbose = TRUE, se = FALSE, rate = FALSE, init_values = NULL) {
    ...
    stop.time <- Sys.time()
    minutes <- difftime(stop.time, start.time, units = "secs")
